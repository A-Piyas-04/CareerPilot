...
import { Bell, BellRing, RefreshCw, Sparkles, X } from "lucide-react";
...
function NotificationListIcon({ notification }: { notification: WorkspaceNotification }) {
  ...
  return (
    <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-sky-500 to-indigo-700 text-white">
...
export function AiNudgeBell() {
  ...
  const hasDueReminders = panelNotifications.some(
    (item) => item.kind === "due-reminder",
  );
  ...
  const count = panelNotifications.length;
        ...
        >
          ...
          <div className="max-h-[min(24rem,calc(100dvh-8rem))] overflow-y-auto">
            {isLoading ? (
              ...
              <p className="px-4 py-8 text-center text-sm text-[var(--cp-text-muted)]">
                You&apos;re all caught up. No reminders or AI suggestions right
                now.
              </p>
            ) : (
                ...
                {panelNotifications.map((notification) => (
                  <li key={notification.id} className="p-4">
                    <div className="flex items-start gap-3">
                      <NotificationListIcon notification={notification} />
                      <div className="min-w-0 flex-1">
                        ...
                        <Link
                          ...
                          onClick={() => {
                            dismissFromPanel(notification.id);
                            setOpen(false);
                          }}
                        >
                      ...
                      <button
                        ...
                        onClick={() => dismissFromPanel(notification.id)}
          ...
          !notificationsEnabled ? (
            <div className="border-t border-[var(--cp-border-soft)] px-4 py-3">
              <button
                type="button"

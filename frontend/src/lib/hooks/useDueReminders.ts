...
function applicationTitle(row: Record<string, unknown>) {
  ...
  const title = nullableString(row.manual_job_title) || nullableString((job as Record<string, unknown> | undefined)?.title);
  const company =
    nullableString(row.manual_company) || nullableString((job as Record<string, unknown> | undefined)?.company);
